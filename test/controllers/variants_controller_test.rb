...
class VariantsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create variant" do
    assert_difference('Variant.count') do
      post variants_url, params: { variant: { belongs_to: @variant.belongs_to, name: @variant.name } }
    end
  ...
  test "should update variant" do
    patch variant_url(@variant), params: { variant: { belongs_to: @variant.belongs_to, name: @variant.name } }
